...
function withErrorBoundary(controller13, defaultCategory = "INTERNAL_SDK_ERROR") {
  const newController = {};
  Object.keys(controller13).forEach((key) => {
    const original = controller13[key];
...
var controller2 = {
  ...
  warn(title, description, code) {
    state5.open = true;
    state5.message = title;
    state5.variant = "warning";
    if (description) {
...
var controller9 = {
  ...
  async connectWalletConnect({ cache = "auto" } = {}) {
    ...
    if (cache === "always" || cache === "auto" && isInTelegramOrSafariIos) {
      ...
      if (!CoreHelperUtil.isPairingExpired(state15?.wcPairingExpiry)) {
...
var controller11 = {
  ...
  getActiveCaipNetwork(chainNamespace) {
    if (chainNamespace) {
      return state17.chains.get(chainNamespace)?.networkState?.caipNetwork;
    }
  ...
  checkIfSupportedNetwork(namespace, caipNetworkId) {
    const activeCaipNetworkId = caipNetworkId || state17.activeCaipNetwork?.caipNetworkId;
...
var controller12 = {
  ...
  addAddressLabel(address, label, chain) {
    ...
    const map = ChainController.state.chains.get(chain)?.accountState?.addressLabels || /* @__PURE__ */ new Map();
...
//# sourceMappingURL=chunk-XR7HJDM7.js.map
