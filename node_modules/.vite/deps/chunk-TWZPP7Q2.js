...
var ConnectionManager = class {
  ...
  async syncBitcoinConnections({ connectors, caipNetwork, universalProvider, getConnectionStatusInfo, onConnection, onListenProvider }) {
    await Promise.all(connectors.filter((c) => {
      ...
      let accounts = addresses?.map((a) => CoreHelperUtil.createAccount(ConstantsUtil.CHAIN.BITCOIN, a.address, a.purpose || "payment", a.publicKey, a.path));
...
var AppKitBaseClient = class {
  ...
  async initialize(options) {
    ...
    if (OptionsController.state.features?.reownAuthentication || OptionsController.state.remoteFeatures?.reownAuthentication) {
      const { ReownAuthentication } = await import("./features-ILCD2N4E.js");
...
var AppKit = class extends AppKitBaseClient {
  ...
  async syncAuthConnector(provider, chainNamespace) {
    ...
    if (chainNamespace && isAuthSupported && shouldSync) {
      ...
      ChainController.setSmartAccountEnabledNetworks(enabledNetworks?.smartAccountEnabledNetworks || [], chainNamespace);
  ...
  async loadModalComponents(features, remoteFeatures) {
    ...
    if (usingEmbeddedWallet) {
      featureImportPromises.push(import("./embedded-wallet-ZZD45OAU.js"));
    }
    if (remoteFeatures.email) {
      featureImportPromises.push(import("./email-YYQLO74Q.js"));
    }
    if (remoteFeatures.socials) {
      featureImportPromises.push(import("./socials-TF4VT3W6.js"));
    }
    if (remoteFeatures.swaps && remoteFeatures.swaps.length > 0) {
      featureImportPromises.push(import("./swaps-6PERS5EF.js"));
    }
    if (features.send) {
      featureImportPromises.push(import("./send-SR3XFHTY.js"));
    }
    if (features.receive) {
      featureImportPromises.push(import("./receive-43HOLMGS.js"));
    }
    if (remoteFeatures.onramp && remoteFeatures.onramp.length > 0) {
      featureImportPromises.push(import("./onramp-PCEM23FC.js"));
    }
    if (remoteFeatures.payWithExchange) {
      featureImportPromises.push(import("./pay-with-exchange-GUMZEC46.js"));
    }
    if (remoteFeatures.activity) {
      featureImportPromises.push(import("./transactions-G5V56WJP.js"));
    }
    if (features.pay || remoteFeatures.payments) {
      featureImportPromises.push(import("./exports-AGT6A5HS.js"));
    }
    if (remoteFeatures.emailCapture) {
      featureImportPromises.push(import("./data-capture-2EU23YSQ.js"));
    }
    await Promise.all([
      ...featureImportPromises,
      import("./exports-42UE3BTA.js"),
      import("./w3m-modal-77GHEJXM.js")
    ]);
  }
};
...
//# sourceMappingURL=chunk-TWZPP7Q2.js.map
