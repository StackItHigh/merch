...
async function recoverPublicKey({ hash: hash2, signature }) {
  ...
  const { secp256k1: secp256k12 } = await import("./secp256k1-A7BM3XF7.js");
  const signature_ = (() => {
...
var EnsInvalidChainIdError = class extends BaseError {
  constructor({ chainId }) {
...
function ripemd1603(value, to_) {
  const to = to_ || "hex";
  const bytes = ripemd1602(isHex(value, { strict: false }) ? toBytes(value) : value);
  if (to === "bytes")
    return bytes;
  return toHex(bytes);
}
...
async function getWebSocketRpcClient(url, options = {}) {
  ...
  return getSocketRpcClient({
    async getSocket({ onClose, onError, onOpen, onResponse }) {
      const WebSocket = await import("./native-LCFHJWJX.js").then((module) => module.WebSocket);
...
function parseErc8010Signature(signature) {
  if (!isErc8010Signature(signature))
    return { signature };
  const { authorization: authorization_ox, to, ...rest } = SignatureErc8010_exports.unwrap(signature);
  return {
    authorization: {
...
function serializeErc8010Signature(parameters) {
  const { address, data, signature, to = "hex" } = parameters;
  const signature_ = SignatureErc8010_exports.wrap({
    authorization: {
...
//# sourceMappingURL=chunk-74XPCBSZ.js.map
