...
import {
  ...
  toHex,
  validate
} from "./chunk-74XPCBSZ.js";
import {
  AccountNotFoundError,
  formatLog,
  formatTransactionReceipt
} from "./chunk-A3WVH42Z.js";
import {
...
import {
  ...
  mapToCurveSimpleSWU,
  secp256k1
} from "./chunk-CQUTSRWJ.js";
import {
  sha256,
  sha384,
  sha512
} from "./chunk-F3M2F5R6.js";
import {
  utf8ToBytes
} from "./chunk-5UBV5ZHI.js";
import {
  __export
...
function weierstrassN(params, extraOpts = {}) {
  ...
  function finishEndo(endoBeta, k1p, k2p, k1neg, k2neg) {
    k2p = new Point3(Fp3.mul(k2p.X, endoBeta), k2p.Y, k2p.Z);
  ...
  class Point3 {
    ...
    static fromBytes(bytes) {
      const P = Point3.fromAffine(decodePoint(_abytes2(bytes, void 0, "point")));
      P.assertValidity();
      return P;
    }
    static fromHex(hex) {
      return Point3.fromBytes(ensureBytes("pointHex", hex));
    }
    get x() {
    ...
    negate() {
      return new Point3(this.X, Fp3.neg(this.Y), this.Z);
    }
...
function ecdh(Point3, ecdhOpts = {}) {
  ...
  const lengths = Object.assign(getWLengths(Point3.Fp, Fn), { seed: getMinHashLength(Fn.ORDER) });
  ...
  return Object.freeze({ getPublicKey: getPublicKey2, getSharedSecret, keygen, Point: Point3, utils: utils2, lengths });
}
function ecdsa(Point3, hash2, ecdsaOpts = {}) {
  ahash(hash2);
  ...
  const hmac3 = ecdsaOpts.hmac || ((key, ...msgs) => hmac(hash2, key, concatBytes(...msgs)));
  const { Fp: Fp3, Fn } = Point3;
  const { ORDER: CURVE_ORDER, BITS: fnBits } = Fn;
  const { keygen, getPublicKey: getPublicKey2, getSharedSecret, utils: utils2, lengths } = ecdh(Point3, ecdsaOpts);
  ...
  class Signature {
    ...
    recoverPublicKey(messageHash) {
      ...
      const R = Point3.fromBytes(concatBytes(pprefix((rec & 1) === 0), x));
  ...
  function prepSig(message, privateKey, opts) {
    ...
    if (extraEntropy3 != null && extraEntropy3 !== false) {
      const e = extraEntropy3 === true ? randomBytes3(lengths.secretKey) : extraEntropy3;
...
function weierstrassPoints(c) {
  ...
  return _weierstrass_new_output_to_legacy(c, Point3);
}
...
function _weierstrass_new_output_to_legacy(c, Point3) {
  const { Fp: Fp3, Fn } = Point3;
...
function _ecdsa_new_output_to_legacy(c, _ecdsa) {
  ...
  return Object.assign({}, _ecdsa, {
    ProjectivePoint: Point3,
    CURVE: Object.assign({}, c, nLength(Point3.Fn.ORDER, Point3.Fn.BITS))
  });
}
function weierstrass(c) {
  const { CURVE, curveOpts, hash: hash2, ecdsaOpts } = _ecdsa_legacy_opts_to_new(c);
  const Point3 = weierstrassN(CURVE, curveOpts);
  const signs = ecdsa(Point3, hash2, ecdsaOpts);
...
function taggedHash(tag, ...messages) {
  ...
  return sha2562(concatBytes(tagP, ...messages));
}
...
function fromDerHex(signature) {
  const { r, s } = secp256k12.Signature.fromDER(from3(signature).slice(2));
  return { r, s };
}
...
function toTuple(signature) {
  ...
  return [
    yParity ? "0x01" : "0x",
    r === 0n ? "0x" : trimLeft(fromNumber2(r)),
    s === 0n ? "0x" : trimLeft(fromNumber2(s))
  ];
}
...
function sha2564(value, options = {}) {
  const { as = typeof value === "string" ? "Hex" : "Bytes" } = options;
  const bytes = sha2563(from2(value));
  if (as === "Bytes")
    return bytes;
  return fromBytes(bytes);
}
...
function createSWU(Point3, opts) {
  const map = mapToCurveSimpleSWU2(Point3.Fp, opts);
...
function getAuthenticatorData(options = {}) {
  ...
  const rpIdHash = sha2564(fromString4(rpId));
...
var BLAKE3 = class _BLAKE3 extends BLAKE2 {
  ...
  b2Compress(counter, flags, buf, bufPos = 0) {
    ...
    const { v0, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12, v13, v14, v15 } = compress2(B3_SIGMA, bufPos, buf, 7, s[0], s[1], s[2], s[3], s[4], s[5], s[6], s[7], B3_IV[0], B3_IV[1], B3_IV[2], B3_IV[3], h, l, pos, flags);
    s[0] = v0 ^ v8;
    s[1] = v1 ^ v9;
    s[2] = v2 ^ v10;
    s[3] = v3 ^ v11;
  ...
  b2CompressOut() {
    ...
    swap32IfBE(buffer32);
    const { v0, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12, v13, v14, v15 } = compress2(B3_SIGMA, 0, buffer32, 7, s[0], s[1], s[2], s[3], s[4], s[5], s[6], s[7], B3_IV[0], B3_IV[1], B3_IV[2], B3_IV[3], l, h, pos, flags);
    out32[0] = v0 ^ v8;
    out32[1] = v1 ^ v9;
    out32[2] = v2 ^ v10;
    out32[3] = v3 ^ v11;
...
function bls(CURVE) {
  ...
  const G1 = Object.assign(G1_, createHasher3(G1_.Point, CURVE.G1.mapToCurve, {
    ...CURVE.htfDefaults,
    ...CURVE.G1.htfDefaults
  }));
  const G2_ = weierstrassPoints(CURVE.G2);
  const G2 = Object.assign(G2_, createHasher3(G2_.Point, CURVE.G2.mapToCurve, {
    ...CURVE.htfDefaults,
  ...
  function sign5(message, privateKey, htfOpts) {
...
var _Field2 = class {
  ...
  isValid({ c0, c1 }) {
    function isValidC(num2, ORDER) {
      return typeof num2 === "bigint" && _0n8 <= num2 && num2 < ORDER;
    }
...
var COMPZERO = setMask(Fp.toBytes(_0n9), { infinity: true, compressed: true });
...
function signatureG1FromBytes(hex) {
  ...
  if (y * _2n7 / P !== aflag)
    y = Fp.neg(y);
  const point = Point3.fromAffine({ x, y });
  point.assertValidity();
  return point;
}
function pointG2ToBytes(_c, point, isComp) {
  ...
  if (isComp) {
    if (is0)
      return concatBytes(COMPZERO, numberToBytesBE(_0n9, L));
...
function pointG2FromBytes(bytes) {
  ...
  const slc = (b, from15, to) => bytesToNumberBE(b.slice(from15, to));
...
var integerToCharacter2 = Object.fromEntries(Array.from("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/").map((a, i) => [i, a.charCodeAt(0)]));
...
var hashToCurve3 = (() => p256_hasher2.hashToCurve)();
var encodeToCurve3 = (() => p256_hasher2.encodeToCurve)();
...
var MASTER_SECRET2 = utf8ToBytes("Bitcoin seed");

// node_modules/viem/node_modules/@scure/bip39/esm/wordlists/czech.js
var wordlist11 = `abdikace
abeceda
adresa
agrese
akce
...
// node_modules/viem/node_modules/ox/_esm/erc4337/UserOperation.js
function from12(userOperation, options = {}) {
  const signature = (() => {
...
*/
//# sourceMappingURL=chunk-JBRICWIW.js.map
